...
import sendHighEmissionAlert from "../services/sendHighEmissionAlert.js"; // ✅ FIXED
...
router.post("/check-total", async (req, res) => {
  ...
  try {
    ...
    if (!settings.enabled) {
      return res.json({
        message: "Alerts disabled by admin",
      });
    }
    ...
    // Load or create EMISSION alert record
    ...
    if (total >= settings.alert10 && !userAlert.alert10Sent) {
      await sendHighEmissionAlert(
        userEmail,
        total,
        `${settings.alert10} KG LIMIT REACHED`
      );
    ...
    if (total >= settings.alert15) {
      await sendHighEmissionAlert(
        userEmail,
        total,
        `${settings.alert15} KG CRITICAL LIMIT`
      );
      alert15Sent = true;
    }
