...
export default function SemiGauge({
  value,
  alert10 = 50,   // warning threshold (admin)
  alert15 = 100,  // critical threshold (admin)
}) {

  /* ===============================
     1️⃣ SCALE GAUGE DYNAMICALLY
     =============================== */
  const max = alert15; // critical limit = max gauge value
  const percent = Math.min(value / max, 1);
  const angle = -90 + percent * 180;

  /* ===============================
     2️⃣ RISK LOGIC (ADMIN-BASED)
     =============================== */
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      {/* ===== THRESHOLD INFO (OPTIONAL BUT NICE) ===== */}
      <p className="text-xs text-gray-500">
        Warning ≥ {alert10} kg | Critical ≥ {alert15} kg
      </p>
    </div>
  );
}
